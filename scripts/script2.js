$(document).ready(function(){
  ...
  function searchArtist(artist){
      artist = $('#title').val();
      callAjax("search?q=", artist, showNames, '&type=artist', 'artists')
    }
  ...
  function getAlbums(){
      callAjax("artists/", $(this).context.id, showAlbums, '/albums')
  }
  ...
  function showAlbums(albums){
      ...
      albums.forEach(album => {
        ...
        if(nameArray.indexOf(name.toLowerCase()) === -1){
                   ...
                   input.type                  = 'button';
                   input.style.backgroundImage = `url(${image})`
                   $(input).addClass('tracks')
                   $(input).attr('id', id)
  ...
  function showTracks(){
    callAjax("albums/", this.id, populatePlayerArr, '/tracks')
  ...
  function callAjax(url, variable, func, optionUrl, resultKey){
    optionUrl = optionUrl || ""
    $.ajax({
      url: "https://api.spotify.com/v1/" + url + variable + optionUrl}).done(function (results) {
